...
class StealthArcher(Archer):
    ...
    def deal_damage(self, hit):
        if(self.alive == False):
            return
        if self.init_time + 10 < time.time():
            self.health -= hit
        if self.health <= 0:
            self.health = 0
            self.kill()
    ...
    def move(self, pos):
        ...
        if(r+c ==1 ):
            ...
            if(pos[0] > self.position[0]):
                    ...
                    self.position[0] -= 1
        self.attack_target(pos)

    def attack_target(self, target):
        if(abs(target[0] - self.position[0]) + abs(target[1] - self.position[1]) > self.attack_radius):
            return
        troops = [barbarians, archers, dragons, balloons, stealth_archers]
        for troop in troops:
            for t in troop:
                for i in range(-self.heal_radius, 1+self.heal_radius):
                    for j in range(-self.heal_radius, 1+self.heal_radius):
                        if(t.position[0] == target[0] + i and t.position[1] == target[1] + j):                              
                            t.health += self.attack
...
def spawnStealthArcher(pos):
    ...
    stealth_archer.init_time = time.time()
...
def move_healers():
    for hel in healers:
        if(hel.alive == False):
            continue
        closest_troop = search_for_closest_troop(hel.position)
        if(closest_troop == None):
            continue
        if(abs(closest_troop[0] - hel.position[0]) + abs(closest_troop[1] - hel.position[1]) > hel.attack_radius):
            hel.move(closest_troop)
        else:
            hel.attack_target(closest_troop)
...
def search_for_closest_troop(pos):
    ...
    for i in range(len(troops)):
        for j in troops[i]:
            if(j.alive == False):
                continue
            dist = abs(j.position[0] - pos[0]) + abs(j.position[1] - pos[1])
            if(dist < closest_dist):
                flag = 1
                closest_dist = dist
                closest_troop = j.position[0], j.position[1]
            if(flag == 0):
                return None
            else:
