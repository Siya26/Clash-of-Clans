            ...
            if self.level >= 3:
                ...
                for troop in troops:
                    for t in troop:
                        for i in range(-2, 3):
                            for j in range(-2, 3):
                                if(t.position[0] == self.position[0] + i and t.position[1] == self.position[1] + j):                              
                                    t.health -= 200
                                    if(t.health < 0):
                                        t.health = 0
                                        t.kill()
...
class WizardTower(Building):
    def __init__(self, position, V):
        ...
        self.V = V
        self.destroyed = False
        self.level = V.wizard_tower_level
        self.health = 60
        self.max_health = 60 + (30 * self.level)
